...
if [[ -z ${SCAN_PATH} ]]; then
    echo '::error ::Specify "scan_path" input'
    exit 1
fi
...
# If scan_path matches remote repo format:
    ...
    if [[ -z ${GITHUB_TOKEN} ]]; then
        echo '::error ::Input "github_token" is needed for scanning remote repository'
        exit 1  
    fi  
else
    echo "Input: ${SCAN_PATH} is invalid. Must be one of directory / file / remote repository with github workflows"
    exit 1
fi
